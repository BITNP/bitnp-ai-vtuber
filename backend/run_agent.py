...
parser.add_argument("--agent-type", default="basic_chatting_agent", choices=["basic_chatting_agent", "lecture_agent", "online_teacher_agent"], help="agent type")
...
parser.add_argument("--command-json", default="script_generator/output/command.json", help="path to command.json for online_teacher_agent")
...
elif args.agent_type == "online_teacher_agent":
    command_json_path = os.path.join(curr_dir, args.command_json)
    agent = create_agent(
        agent_type = "online_teacher_agent",
        server_url = server_url,
        agent_name = agent_name,
